...
pub struct InputState {
    ...
    /// Extra highlight spans (Differ's diff tints) composed with the syntax /
    /// LSP / diagnostic layers at paint time. Byte ranges into the text; usually
    /// only `background_color` is set, so they layer under the syntax colours.
    pub(crate) diff_highlights: Vec<(std::ops::Range<usize>, gpui::HighlightStyle)>,
...
impl InputState {
    ...
    /// Set the diff-highlight spans (Differ). Each is a byte range + a
    /// `HighlightStyle` (typically only `background_color`); they compose with
    /// the syntax/LSP/diagnostic layers via `combine_highlights` at paint time,
    /// so tints render inside the editor and track scrolling natively.
    pub fn set_diff_highlights(
        &mut self,
    ...
    /// Diff-highlight spans clipped to `range` (used by the visible-range paint).
    pub(crate) fn diff_highlights_for_range(
        &self,
