...
impl TextElement {
    ...
    fn highlight_lines(
        ...
        // Differ: diff tints as a base layer. They set only background_color, so
        // they compose under the foreground syntax/diagnostic colours.
        let diff_styles = state.diff_highlights_for_range(&visible_byte_range);
        if !diff_styles.is_empty() {
            styles = gpui::combine_highlights(diff_styles, styles).collect();
        }
