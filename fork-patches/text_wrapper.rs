...
impl LineLayout {
    ...
    pub(crate) fn paint(
        ...
        for (ix, line) in self.wrapped_lines.iter().enumerate() {
            let line_pos = pos + point(px(0.), ix * line_height);
            // Run backgrounds (diff tints etc.) must be painted explicitly —
            // ShapedLine::paint only draws glyphs, not run backgrounds.
            _ = line.paint_background(line_pos, line_height, text_align, align_width, window, cx);
            _ = line.paint(line_pos, line_height, text_align, align_width, window, cx);
        }
